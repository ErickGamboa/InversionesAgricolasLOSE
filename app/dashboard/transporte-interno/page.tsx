...
export default function TransporteInternoPage() {
  ...
  const { data: transactions = [], mutate, isLoading } = useSWR("transporte_interno", fetcher);
  const { data: lookups } = useSWR("lookups_interno", fetchLookups);
  ...
  const handleSubmit = useCallback(async (formData: Record<string, unknown>) => {
    ...
    try {
      ...
      if (editingTransaction) {
        ...
        const { error } = await supabase.from("transporte_interno").insert(payload);
        if (error) throw error;
  ...
  return (
    <>
      ...
      <div className="space-y-6 p-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Registro de transporte interno de piña</p>
          </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Historial de Transporte Interno</CardTitle>
        </CardHeader>
        <CardContent>
          <TransactionTable
      ...
      <Dialog open={isDialogOpen} onOpenChange={(open) => { setIsDialogOpen(open); if (!open) setEditingTransaction(null); }}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>
              {editingTransaction ? "Editar Registro" : "Nuevo Transporte Interno"}
            </DialogTitle>
          </DialogHeader>
          <TransactionForm
