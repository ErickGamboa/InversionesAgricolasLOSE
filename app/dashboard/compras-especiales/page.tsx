...
export default function ComprasEspecialesPage() {
  ...
  const { data: transactions = [], mutate, isLoading } = useSWR("compras_especiales", fetcher);
  const { data: lookups } = useSWR("lookups_especiales", fetchLookups);
  ...
  const handleSubmit = useCallback(async (formData: Record<string, unknown>) => {
    ...
    try {
      ...
      if (editingTransaction) {
        ...
        const { error } = await supabase.from("compras_especiales").insert(payload);
        if (error) throw error;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Registro de compras especiales de pina</p>
        </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={(open) => { setIsDialogOpen(open); if (!open) setEditingTransaction(null); }}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>
              {editingTransaction ? "Editar Compra" : "Nueva Compra Especial"}
            </DialogTitle>
          </DialogHeader>
          <TransactionForm
